...
int main(void){
	...
	while(1){
		if(readAcc(data)){
			_delay_ms(2);
			for( j=0;j<3;j++){
				itoa(data[j],myString,10);
				uart_transmitMult(myString);
				if(j<2)uart_transmit(',');
			}
			uart_transmit('\r');
		}
	}
...
void acc_init(){
	uint8_t value =1;
	i2c_write(ACC_addr,1,0x2A,value);
}

void readAcc(int16_t *buf){
	uint8_t readings[6]={0};
	i2c_read(ACC_addr,1,0x00,readings[0]);
	if(readings[0]){
		i2c_read(ACC_addr,6,0x01,readings);
		uint8_t i;
		for(i=0;i<3;i++){
			uint16_t temp = readings[2*i]<<8 | readings[2*i + 1];
			if(readings[2*i]>0x7F){
				temp=~temp + 1;
				buff[i]=-temp;
			}else{
				buff[i]=temp;
