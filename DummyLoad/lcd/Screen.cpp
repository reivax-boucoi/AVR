...
template <class T> Value<T>::Value(T* v, int x, int y, int digits) {
  val = v;
  posX = x;
  posY = y;
  dispVal = *val;
...
template <class T> Value<T>::Value(T* v, int x, int y, int digits, T min, T max) {
  val = v;
  posX = x;
  posY = y;
  nbDigits = digits;
...
template <class T> void Value<T>::show(void) {
  if (dispVal < 10.0) {
    DPpos = nbDigits - 1;
...
template <class T> void Value<T>::up(void) {
...
template <class T> void Value<T>::down(void) {
  int p = nbDigits - DPpos - 1 - cursorPos;
  (*val) -=  pow(10, p);
  if ((*val) < minVal)(*val) = minVal;
  dispVal = (*val);
...
template <class T> void Value<T>::advanceCursor(void) {
  cursorPos++;
  if (cursorPos >= nbDigits)cursorPos = 0;
}

template <class T> void Value<T>::showCursor(bool editing) {
  if (!editing) {
...
Screen::Screen(const char* line1, const char* line2, AbstractValue *value0, AbstractValue *value1, AbstractValue *value2, AbstractValue *value3) {
  values[3] = value3;
  values[2] = value2;
  values[1] = value1;
  values[0] = value0;
